...
class SummaryAgent:
    ...
    def compile_analysis(self, alert: Dict, agent_results: List[Dict], user_prompt: str) -> Dict[str, Any]:
        """Main function: Structured 3-part incident report (summary, evidence, recommendations)"""
        # 1. Filter agent_results: only successful, non-empty, non-error agents
        filtered_results = []
        for r in agent_results:
            if r.get('status') == 'failed':
                continue
            if r.get('agent_type') == 'summary':
                continue
            # Skip empty/meaningless results
            if 'analysis' in r and isinstance(r['analysis'], dict):
                findings = r['analysis'].get('findings', [])
                if not findings or all(not f or f.strip() == '' for f in findings):
                    continue
            filtered_results.append(r)
        # 2. Build agent status for summary narrative
        included_agents = self._extract_included_agents(filtered_results)
        agent_status = self._normalize_agent_labels(filtered_results, included_agents)
        overall_risk_level, overall_risk_score, _ = self._calculate_overall_risk(agent_status)
        # 3. Generate summary (narrative)
        summary = self._generate_story_narrative(alert, filtered_results, agent_status, overall_risk_level, overall_risk_score, user_prompt)
        # 4. Extract evidence from timeline_agent (chronological events)
        evidence = []
        for result in filtered_results:
            if result.get('agent_type') == 'timeline' and result.get('timeline_events'):
                for event in result['timeline_events']:
                    if event.get('timestamp') and event.get('description'):
                        evidence.append({
        ...
        recommendations = self._generate_recommendations(overall_risk_level, filtered_results)
        # 6. Return only the required fields, always present and clean
        return {
            "summary": summary,
            "evidence": evidence,
    ...
    def _extract_included_agents(self, agent_results: List[Dict]) -> List[str]:
        """Extract list of agents that actually executed and produced valid results (no failed/empty/self)."""
        included = []
    ...
    def _apply_label_precedence(self, result: Dict, agent_type: str) -> tuple:
        """Apply precedence rules to derive status_label, always default to NORMAL if missing/empty."""
        # Rule 1: If agent already provides string risk level, map aliases
        risk_level = result.get('risk_level', '').strip().upper() if isinstance(result.get('risk_level'), str) else ''
        if risk_level:
            risk_mapping = {
        ...
        if not status_label or status_label.strip() == '':
            status_label = 'NORMAL'
        reason = self._generate_reason_from_result(result, agent_type)
    ...
    def _infer_from_content(self, result: Dict, agent_type: str) -> str:
        ...
        if agent_type == 'timeline':
            ...
            threat_assessment = (result.get('threat_assessment') or 'UNKNOWN').upper()
    ...
    def _generate_reason_from_result(self, result: Dict, agent_type: str) -> str:
        """Generate one-line factual reason from agent findings or fallback to agent type."""
        # Try to extract key findings
        if 'analysis' in result and isinstance(result['analysis'], dict):
            findings = result['analysis'].get('findings', [])
            for finding in findings:
                if finding and finding.strip():
                    return f"{finding[:60]}..." if len(finding) > 60 else finding
        ...
        if 'report' in result:
            ...
            for line in report_lines:
                if line.strip() and 'FINDINGS:' in line.upper():
                    ...
                    for next_line in report_lines[idx+1:]:
        ...
        if agent_type == 'user_context':
            ...
            return "IP analysis completed."
        elif agent_type == 'timeline':
            return "Timeline patterns analysis completed."
        elif agent_type == 'alert_history':
    ...
    def _generate_story_narrative(self, alert: Dict, agent_results: List[Dict], 
        ...
        """Generate concise, human-friendly, 3–4 sentence executive summary from valid agent results only."""
        ...
        # Section 3: What we found (from valid agent_status only)
        findings_lines = []
        for agent, status_info in agent_status.items():
            if status_info['status_label'] != 'N/A':
