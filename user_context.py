...
class UserContextAgent:
    ...
    def analyze_query_intent(self, analysis_prompt: str) -> List[str]:
        """Analyze the analysis prompt to determine what type of user queries are needed"""
    ...
    def generate_dynamic_queries(self, user_context: Dict, analysis_categories: List[str], analysis_prompt: str) -> List[Dict]:
        """Generate queries based on available context and analysis intent"""
        queries = []
        ...
        for category in analysis_categories:
            if category == 'authentication_analysis':
                queries.append({
                    'name': f'authentication_patterns_{username}',
                    'prompt': f"Analyze authentication patterns and login behavior for user {username} including successes and failures",
                    'focus': 'authentication behavior'
                })
            
            elif category == 'activity_analysis':
                queries.append({
                    'name': f'user_activities_{username}',
                    'prompt': f"Find all user activities and behavioral patterns for {username} to identify anomalies",
                    'focus': 'user activity patterns'
                })
            
            elif category == 'privilege_analysis':
                queries.append({
                    'name': f'privilege_analysis_{username}',
                    'prompt': f"Investigate privilege changes and access modifications for user {username}",
                    'focus': 'privilege escalation'
                })
            
            elif category == 'temporal_analysis':
                queries.append({
                    'name': f'temporal_patterns_{username}',
                    'prompt': f"Analyze time-based patterns and schedule anomalies for user {username}",
                    'focus': 'temporal behavior'
                })
            
            elif category == 'account_analysis':
                queries.append({
                    'name': f'account_changes_{username}',
                    'prompt': f"Find account modifications and changes for user {username}",
                    'focus': 'account management'
                })
            
            elif category == 'network_analysis':
                queries.append({
                    'name': f'network_behavior_{username}',
                    'prompt': f"Investigate network behavior and remote access patterns for user {username}",
    ...
    def analyze_user(self, alert: Dict, data_entry: Dict, analysis_prompt: str) -> Dict[str, Any]:
        """Main function: Complete user context analysis"""
        
        # Extract context dynamically
        user_context = self.extract_user_context(alert, data_entry)
        
        # Analyze intent 
        analysis_categories = self.analyze_query_intent(analysis_prompt)
        
        # Generate queries dynamically
        queries = self.generate_dynamic_queries(user_context, analysis_categories, analysis_prompt)
